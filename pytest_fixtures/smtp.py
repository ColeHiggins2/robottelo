...
from robottelo.net import my_ip_address
from robottelo.net import net_check_remote
from robottelo.net import random_port
from robottelo.smtp_collector_client import SMTPCollectorClient
...
def smtp_test(request, appliance):
    ...
    appliance.server.settings.update_smtp_server(
        {'host': my_ip, 'port': str(mail_server_port), 'auth': "none"}
    )
    ...
    """
    assert (
        collector.poll() is None
    ), "Collector has died. Something must be best.Server() as mt:
    send_welcome_email()locking selected ports"
    """
    ...
    assert server_port_open, (
        f"The destination {my_ip} port {mail_server_port} seem "
