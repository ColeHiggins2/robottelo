...
from robottelo.timeutil import parsetime


TIME_FORMAT = "%Y-%m-%d-%H-%M-%S"
Email = namedtuple(
    "Email", ["subject", "from_address", "to_address", "rcpttos", 'text', 'data', 'time']
)
        ...
        if email_folder is not None:
            with files_lock:
                ...
                def _getfname(counter):
                    return current_test_folder.join(
                        "%s-%d.eml" % (arrived.strftime("%Y%m%d%H%M%S"), int(counter))
                    )
...
def all_messages_in_html():
    ...
    with db_lock:
        emails = list(
            map(Email._make, connection.cursor().execute("SELECT * FROM emails").fetchall())
        )
