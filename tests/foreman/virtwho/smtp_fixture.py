...
RecordedMessage = namedtuple('RecordedMessage', 'peer envelope_from envelope_recipients data',)
...
class SMTPServerThread(Thread):
    ...
    def run(self):
        ...
        self.smtp = _SMTPServer(('0.0.0.0', 0), None)
...
# @pytest.fixture
# def sat_setup(request):
# Updating Satellite smtp server
# ssh.command(f"hammer settings set --name smtp_address --value '{smtp_server}'")
# ssh.command(f"hammer settings set --name smtp_port --value '{smtp_port}'")
